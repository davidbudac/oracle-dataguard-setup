...
> ## ⚠️ Historical record — do not read as a description of current code
>
> This document came from the branch `fix/bugfix-review-2026-04-19`, which was
> **never merged**. It is preserved here (2026-07-17) because it is the only
> written account of *why* this class of bug matters, and that reasoning is
> still worth having.
>
> **Every bug described below is fixed on `main`** — but by later, independent
> implementations, not by the patches described here. Three of them now have
> dedicated regression tests: `((VAR++))` under `set -e`
> (`tests/test_counter_increment.sh`), `df -k` / AIX portability
> (`tests/test_df_parsing.sh`), and unanchored path substitution
> (`tests/test_path_token_remap.sh`).
>
> **The file and line references below are stale** — they point into the April
> branch, not `main`. Use this document for the *rationale*; use the tests and
> `CLAUDE.md` for what the code does today.
