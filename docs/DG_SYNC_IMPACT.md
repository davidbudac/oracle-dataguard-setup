...
The converter sticks to what AIX 7.2's `/usr/bin/awk` accepts - it uses
no function-local array (an unsupplied extra parameter is a scalar
there), seeds every global array in `BEGIN`, and keys table cells with a
`"row|col"` string rather than a multi-subscript `arr[i,j]` reference.
That awk rejects all three outright with `0602-558 cannot be used as an
array`, killing the run mid-report. Should the conversion still fail on
some other vendor awk, the
