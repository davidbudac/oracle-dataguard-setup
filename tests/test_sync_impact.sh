...
# array" instead of coping with a function-local array, a name it has not
# already seen subscripted, or a multi-subscript (SUBSEP) reference.
...
assert_contains "column arrays seeded in BEGIN" "$RENDERER" 'numcol[0] = 0; chartcol[0] = 0; cmax[0] = 0'
MULTIDIM=$(printf '%s\n' "$RENDERER" | grep -cE '(tcell|tnc|cells|numcol|chartcol|cmax)\[[^]]*,')
assert_eq "no multi-subscript array references" "0" "$MULTIDIM"
# a subscripted name that is also an extra parameter of the function it
# appears in is the exact construct AIX awk killed the run over
LOCALARR=$(printf '%s\n' "$RENDERER" | awk '
    /^ *function /  { split($0, p, /[(),]/); fn = ""
                      for (k = 2; k in p; k++) { gsub(/[ \t{]/, "", p[k]); if (p[k] != "") fn = fn " " p[k] " " }
                      next }
    { line = $0
...
assert_eq "no function-local arrays" "" "${LOCALARR% }"
