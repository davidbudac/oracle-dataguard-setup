...
if [[ ! -f "${SCRIPT_DIR}/config.env.dgcdb" ]]; then
    echo "ERROR: config.env.dgcdb not found."
    exit 1
fi

source "${SCRIPT_DIR}/config.env.dgcdb"
...
phase_create_db() {
    log_phase "CREATE DB: Creating test primary database (CDB, no PDBs, no OMF, no FRA)"
    ...
    result=$(ssh_cmd "PRIMARY" "
        ...
        # Create CDB with DBCA, no PDBs (handles catalog/catproc automatically)
...
phase_step2() {
    ...
    local result
    # Prompts: standby host, db_unique_name, SID (default), storage mode (default=Traditional), SRL-separate (default N), confirm
...
phase_step6() {
    ...
    # Validate: broker configuration exists and is enabled. Poll QUIETLY
    # (attempts that will be retried must not log_fail) and assert only
    # the final outcome. Real ERE alternation, not BRE \| - the runner
    # host's grep may be ugrep, which treats \| in -E as a literal.
    _broker_ok=0
    _broker_out=""
    ...
    if [[ $_broker_ok -eq 1 ]]; then
        ...
        log_fail "Broker configuration enabled: expected SUCCESS/enabled in output"
...
phase_step11() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        record_issue "step11" "create_role_trigger_cdb.sh failed" "$(echo "$result" | tail -20)"
        return 1
    fi
