...
> - Known limitation found: when the primary keeps data+redo in ONE directory but the standby splits them, first-prefix-match means ORLs/SRLs land in the standby DATA dir, not the split redo dir (both convert strings share one pair list). Functional, but a split needs a distinct primary redo dir too.
