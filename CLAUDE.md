...
- **Standby file locations (Q1b)**: step 2 (Traditional mode, TTY only) asks up front whether the standby uses the **same paths** as the primary (default; DB-name components still renamed) or the **same layout on renamed filesystems** — one suffix (e.g. `_s`) appended to the FIRST path component of every derived location (`/ora_1/oradata -> /ora_1_s/oradata`), applied via `derive_standby_path()` = `remap_path_token()` then `apply_fs_suffix()` to datafiles, redo, SRLs, archive dest, and FRA defaults. The standby `ORACLE_BASE` **default** gets the suffix on its LAST component instead (`/u01/app/oracle -> /u01/app/oracle_s` — the software mount isn't renamed). The whole Q1b block is TTY-gated, so piped/E2E runs get the same-paths scenario (empty suffix = pre-Q1b behavior, byte-identical stdin sequence). `STANDBY_FS_SUFFIX` is persisted to the .env as a record only — the path arrays are the truth and `--regenerate` never re-applies it. `apply_fs_suffix`/`derive_standby_path` are duplicated verbatim in `tests/test_fs_suffix_remap.sh`, whose drift guard diffs the copies (script is the source of truth; keep the function header and closing `}` at column 0)
...
- `tests/test_fs_suffix_remap.sh` - Tests step 2's Q1b filesystem-suffix derivation (`apply_fs_suffix`/`derive_standby_path`: first-component suffix, composition with the token remap, empty-suffix no-op, the ORACLE_BASE last-component rule) with a drift guard diffing both functions against the script
